...
andvalue(VALUE *v1, VALUE *v2, VALUE *vres)
{
    ...
    case TWOVAL(V_STR, V_OCTET):
        vres->v_str = charstring(*v1->v_str->s_str & *v2->v_octet);
        return;
    case TWOVAL(V_OCTET, V_STR):
        vres->v_type = V_STR;
        vres->v_str = charstring(*v1->v_octet & *v2->v_str->s_str);
        return;
    default:
        if ((v1->v_type != V_OBJ) && (v2->v_type != V_OBJ)) {
            if (v1->v_type < 0) {
...
setminusvalue(VALUE *v1, VALUE *v2, VALUE *vres)
{
    ...
    default:
        if (v1->v_type == V_OBJ || v2->v_type == V_OBJ) {
            *vres = objcall(OBJ_SETMINUS, v1, v2, NULL_VALUE);
        } else {
...
contentvalue(VALUE *vp, VALUE *vres)
{
    ...
    case V_NUM:
        ...
        break;
    case V_OBJ:
        *vres = objcall(OBJ_CONTENT, vp, NULL_VALUE, NULL_VALUE);
        return;
    default:
...
conjvalue(VALUE *vp, VALUE *vres)
{
    ...
    case V_COM:
        ...
        qfree(vres->v_com->imag) vres->v_com->real = qlink(vp->v_com->real);
...
shiftvalue(VALUE *v1, VALUE *v2, bool rightshift, VALUE *vres)
{
    ...
    case V_NUM:
        ...
        return;
    case V_COM:
        if (qisfrac(v1->v_com->real) || qisfrac(v1->v_com->imag)) {
            *vres = error_value(E_SHIFT_1);
            return;
        }
...
comparevalue(VALUE *v1, VALUE *v2)
{
    ...
    if (v1->v_type == V_OCTET) {
        ...
        if (v2->v_type == V_STR) {
            return (*v1->v_octet != (OCTET)*v2->v_str->s_str) || (v2->v_str->s_len != 1);
        }
        if (v2->v_type != V_NUM || qisfrac(v2->v_num) || qisneg(v2->v_num) || v2->v_num->num.len > 1) {
            return true;
        }
...
precvalue(VALUE *v1, VALUE *v2)
{
    ...
    relvalue(v1, v2, &val);
    if ((val.v_type == V_NUM && qisneg(val.v_num)) || (val.v_type == V_COM && qisneg(val.v_com->imag))) {
        r = 1;
    }
...
relvalue(VALUE *v1, VALUE *v2, VALUE *vres)
{
    ...
    case V_STR:
        ...
        case V_STR:
            ...
            break;
        case V_OCTET:
            r = (unsigned char)*v1->v_str->s_str - *v2->v_octet;
            if (r == 0) {
                if (v1->v_str->s_len == 0) {
                    r = -1;
                } else {
    ...
    case V_OCTET:
        ...
        case V_OCTET:
            ...
            break;
        case V_STR:
            r = *v1->v_octet - (unsigned char)*v2->v_str->s_str;
            if (r == 0) {
                if (v2->v_str->s_len == 0) {
                    r = 1;
                } else {
...
printvalue(VALUE *vp, int flags)
{
    ...
    case V_LIST:
        if (!userfunc("list_print", vp)) {
            listprint(vp->v_list, ((flags & PRINT_SHORT) ? 0L : conf->maxprint));
        }
        break;
    case V_ASSOC:
        assocprint(vp->v_assoc, ((flags & PRINT_SHORT) ? 0L : conf->maxprint));
        break;
    case V_MAT:
        if (!userfunc("mat_print", vp)) {
            matprint(vp->v_mat, ((flags & PRINT_SHORT) ? 0L : conf->maxprint));
        }
        break;
    case V_FILE:
...
config_print(CONFIG *cfg)
{
    ...
    for (cp = configs; cp->name; cp++) {
        ...
        if (cp->type == CONFIG_ALL || strcmp(cp->name, "maxerr") == 0 || strcmp(cp->name, "ctrl-d") == 0) {
            continue;
        }
