    ...
    for (i = 0; htbl[i].init_state != NULL; ++i) {
        ...
        if (type == htbl[i].type) {
            ...
            if (state->chunksize > MAX_CHUNKSIZE) {
                math_error("internal error: MAX_CHUNKSIZE is too small");
                not_reached();
            }
            return state;
        }
...
hash_value(int type, void *v, HASH *state)
{
    ...
    case V_MAT:
        ...
        for (i = 0; i < value->v_mat->m_size; ++i) {
            ...
            state = hash_value(type, value->v_mat->m_table + i, state);
    ...
    case V_LIST:
        ...
        for (i = 0, ep = value->v_list->l_first; ep != NULL && i < value->v_list->l_count; ++i, ep = ep->e_next) {
    ...
    case V_OBJ:
        ...
        state = hash_str(type, objtypename(value->v_obj->o_actions->oa_index), state);
        (state->chkpt)(state);

        for (i = value->v_obj->o_actions->oa_count, vp = value->v_obj->o_table; i-- > 0; vp++) {
    ...
    case V_CONFIG:
        ...
        state = hash_long(type, (long)value->v_config->epsilonprec, state);
        ...
        state = hash_long(type, (long)value->v_config->maxscancount, state);
        ...
        state = hash_long(type, (long)value->v_config->resource_debug, state);
        state = hash_long(type, (long)value->v_config->calc_debug, state);
        state = hash_long(type, (long)value->v_config->user_debug, state);
        ...
        if (value->v_config->allow_custom != NULL && *(value->v_config->allow_custom)) {
    ...
    case V_HASH:
        ...
        (state->update)(state, value->v_hash->h_union.data, state->unionsize);
        state->bytes = false; /* as if reading words */
        break;
    ...
    case V_BLOCK:
        ...
        if (value->v_block->datalen > 0) {
            state = hash_usb8(type, value->v_block->data, value->v_block->datalen, state);
        }
        break;
    ...
    case V_NBLOCK:
        ...
        if (value->v_nblock->blk->datalen > 0) {
            state = hash_usb8(type, value->v_nblock->blk->data, value->v_nblock->blk->datalen, state);
        }
        break;
