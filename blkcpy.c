...
copystod(VALUE *svp, long ssi, long num, VALUE *dvp, long dsi)
{
    ...
    case V_STR:
        ...
        case V_NBLOCK:
            /* copy to a block */
            return copystr2blk(svp->v_str, ssi, num, dblk, dsi, noreloc);
        case V_FILE:
            return copystr2file(svp->v_str, ssi, num, dvp->v_file, dsi);
        case V_STR:
            return copystr2str(svp->v_str, ssi, num, dvp->v_str, dsi);
        }
        return E_COPY_12;
    ...
    case V_OCTET:
        ...
        case V_BLOCK:
        case V_NBLOCK:
            return copyostr2blk((char *)svp->v_octet, ssi, num, dblk, dsi, noreloc);
        case V_STR:
            return copyostr2str((char *)svp->v_octet, ssi, num, dvp->v_str, dsi);
        }
        return E_COPY_12;
    ...
    case V_NUM:
        ...
        if (dblk != NULL) {
            /* copy to a block */
            return copynum2blk(svp->v_num, ssi, num, dblk, dsi, noreloc);
        }
        switch (dvp->v_type) {
        case V_MAT:
            /* copy to a matrix */
    ...
    case V_FILE:
        ...
        if (dblk != NULL) {
            /* copy to a block */
            return copyfile2blk(svp->v_file, ssi, num, dblk, dsi, noreloc);
        }
        switch (dvp->v_type) {
        case V_NUM:
            /* copy to a number */
    ...
    case V_MAT:
        ...
        if (dblk != NULL) {
            /* copy to a block */
            return copymat2blk(svp->v_mat, ssi, num, dblk, dsi, noreloc);
        }
        switch (dvp->v_type) {
        case V_MAT:
            /* copy to a matrix */
            return copymat2mat(svp->v_mat, ssi, num, dvp->v_mat, dsi);
        case V_LIST:
            /* copy to a list */
            return copymat2list(svp->v_mat, ssi, num, dvp->v_list, dsi);
        }
        break;
    ...
    case V_LIST:
        ...
        case V_MAT:
            /* copy to a matrix */
            return copylist2mat(svp->v_list, ssi, num, dvp->v_mat, dsi);
        case V_LIST:
            /* copy to a list */
            return copylist2list(svp->v_list, ssi, num, dvp->v_list, dsi);
        }
        break;
    }
...
copyblk2num(BLOCK *sblk, long ssi, long num, NUMBER *dnum, long dsi, NUMBER **res)
{
    ...
    memcpy((char *)(ret->num.v + dsi), swapped, (num / sizeof(HALF)) * sizeof(HALF));
    /* copy over any octets in the last partial HALF */
    i = num % sizeof(HALF);
    if (i != 0) {
        memcpy((char *)(ret->num.v + dsi) + num - i, (char *)swapped + num - i, i);
    }
    free(swapped);
#endif
    /* save new number */
