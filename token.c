...
STATIC struct keyword keywords[] = {{"if", T_IF},           {"else", T_ELSE},
                                    ...
                                    {"return", T_RETURN},   {"local", T_LOCAL},
                                    {"global", T_GLOBAL},   {"static", T_STATIC},
                                    {"switch", T_SWITCH},   {"case", T_CASE},
                                    {"default", T_DEFAULT}, {"quit", T_QUIT},
                                    {"exit", T_QUIT},       {"define", T_DEFINE},
                                    {"read", T_READ},       {"show", T_SHOW},
                                    {"help", T_HELP},       {"man", T_HELP},
                                    {"write", T_WRITE},     {"mat", T_MAT},
                                    {"obj", T_OBJ},         {"print", T_PRINT},
...
scanerror(int skip, char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    if (name) {
        snprintf(calc_err_msg, MAXERROR, "\"%s\", line %ld: ", name, linenumber());
    ...
    if ((!c_flag && !stoponerror) || stoponerror > 0) {
        if (calc_use_scanerr_jmpbuf != 0) {
            ...
            fprintf(stderr, "calc_scanerr_jmpbuf not setup, exiting code 60\n");
    ...
    if (conf->maxscancount > 0 && errorcount > conf->maxscancount) {
        ...
        if (calc_use_scanerr_jmpbuf != 0) {
            ...
            fprintf(stderr, "calc_scanerr_jmpbuf not ready: exit 61\n");
            libcalc_call_me_last();
            exit(61);
        }
    }
...
warning(char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    if (name) {
        snprintf(calc_warn_msg, MAXERROR, "\"%s\", line %ld: ", name, linenumber());
