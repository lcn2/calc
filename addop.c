    ...
    if (funccount > 0) {
        ...
        for (index = 0; index < funccount; index++) {
            ...
            if (conf->resource_debug & RSCDBG_FUNC_INFO) {
                ...
                math_fmt("%5ld\t%-12s\t", index, namestr(&funcnames, index));
                if (fp) {
                    count++;
                ...
                count++;
                math_fmt("%-12s\t%-2d\n", namestr(&funcnames, index), fp->f_paramcount);
            }
        }
    }
    if (conf->resource_debug & RSCDBG_FUNC_INFO) {
...
endfunc(void)
{
    ...
    if (errorcount) {
        scanerror(T_NULL, "Compilation of \"%s\" failed: %ld error(s)", newname, errorcount);
        return;
    }
...
addop(long op)
{
    ...
    if ((count + 5) >= maxopcodes) {
        ...
        memcpy((char *)fp, (char *)curfunc, funcsize(curfunc->f_opcodecount));
        if (curfunc != functemplate) {
            free(curfunc);
        }
        curfunc = fp;
