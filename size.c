...
lsizeof(VALUE *vp)
{
    ...
    if (vp->v_type > 0) {
        ...
        case V_RAND:
            ...
            break;
        case V_RANDOM:
            s = sizeof(RANDOM) + zsize(vp->v_random->n) + zsize(vp->v_random->r);
            break;
        case V_CONFIG:
            s = sizeof(CONFIG) + strlen(vp->v_config->prompt1) + strlen(vp->v_config->prompt2) + 2;
            break;
        case V_HASH:
            /* ignore the unused part of the union */
            s = sizeof(HASH) + vp->v_hash->unionsize - sizeof(vp->v_hash->h_union);
            break;
        case V_BLOCK:
...
memsize(VALUE *vp)
{
    ...
    if (vp->v_type > 0) {
        ...
        case V_NUM:
            ...
            break;
        case V_COM:
            s = sizeof(COMPLEX) + memqsize(vp->v_com->real) + memqsize(vp->v_com->imag);
            break;
        case V_STR:
        ...
        case V_LIST:
            s = sizeof(LIST);
            for (ep = vp->v_list->l_first; ep; ep = ep->e_next) {
                s += sizeof(LISTELEM) - sizeof(VALUE) + memsize(&ep->e_value);
            }
            break;
        case V_ASSOC:
            s = sizeof(ASSOC);
            ...
            while (i-- > 0) {
                ...
                for (aep = *ept++; aep; aep = aep->e_next) {
                    s += sizeof(ASSOCELEM) - sizeof(VALUE) + memsize(&aep->e_value);
                    j = aep->e_dim;
                    p = aep->e_indices;
                    while (j-- > 0) {
                        s += memsize(p++);
        ...
        case V_RAND:
            ...
            break;
        case V_RANDOM:
            s = sizeof(RANDOM) + memzsize(vp->v_random->n) + memzsize(vp->v_random->r);
            break;
        case V_CONFIG:
        ...
        case V_BLOCK:
            ...
            break;
        case V_NBLOCK:
            s = sizeof(NBLOCK) + sizeof(BLOCK) + vp->v_nblock->blk->maxsize + strlen(vp->v_nblock->name) + 1;
            break;
        default:
