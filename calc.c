...
main(int argc, char **argv)
{
            ...
            for (;;) {
                ...
                case 'm':
                    ...
                    if (*cp < '0' || *cp > '7') {
                         ...
                         * nothing to cleanup
                         */
                        fprintf(stderr, "%s: unknown -m arg\n", program);
                        exit(3);
                    }
                ...
                case 'D':
                     ...
                     */
                    if (nextcp(&cp, &index, argc, argv, false)) {
                        fprintf(stderr, "-D expects argument\n");
                        exit(5);
                    }
                    havearg = true;
                    if (*cp != ':') {
                        ...
                        cp = endcp;
                        if (*cp != '\0' && *cp != ' ' && *cp != ':') {
                            fprintf(stderr, "Bad syntax im -D"
                                            " arg\n");
                            exit(7);
                        }
                        if (*cp != ':') {
                            if (nextcp(&cp, &index, argc, argv, false) || *cp != ':') {
                                break;
                            }
                        }
                    }
                ...
                case 'f':
                    haveendstr = (cp[1] == '\0');
                    if (nextcp(&cp, &index, argc, argv, haveendstr)) {
                    ...
                    strlcpy(cmdbuf + cmdlen, "read ", sizeof("read "));
                    cmdlen += sizeof("read ") - 1;
                    cmdbuf[cmdlen] = '\0';
                    if (strncmp(cp, "-once", sizeof("-once")) == 0 &&
                        (cp[sizeof("-once") - 1] == '\0' || cp[sizeof("-once") - 1] == ' ')) {
                        cp += sizeof("-once") - 1;
                        haveendstr = (*cp == '\0');
                        strlcpy(cmdbuf + cmdlen, "-once ", sizeof("-once "));
                    ...
                    if (script_name == NULL) {
                        fprintf(stderr, "strdup(-f argument)"
                                        "failed\n");
                        exit(17);
                    }
    ...
    while (run_state == RUN_TOP_LEVEL) {
        ...
        if ((c_flag && !stoponerror) || stoponerror < 0) {
            ...
            if (stdin_tty) {
                reinitialize();
                getcommands(true);
            } else if (inputisterminal() && !p_flag && (!havecommands || i_flag)) {
                closeinput();
                if (!freopen("/dev/tty", "r", stdin)) {
#if defined(_WIN32) || defined(_WIN64)
                    fprintf(stderr, "/dev/tty does not exist on "
     ...
     */
    libcalc_call_me_last();
    return (run_state == RUN_EXIT_WITH_ERROR || run_state == RUN_ZERO) ? 1 : 0;
}
...
{
    if (conf->calc_debug & CALCDBG_RUNSTATE) {
        printf("main: run_state from %s to %s\n", run_state_name(run_state), run_state_name(state));
    }
    run_state = state;
}
