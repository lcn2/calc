...
u_pfe_pipe(char *UNUSED(name), int UNUSED(count), VALUE **UNUSED(vals))
{
	...
	int fds[2];
	if (pipe(fds)) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
valv_optional_get_num(int count, VALUE **vals, int* idx, const char* name, VALUE* dflt) {
	...
	switch (chk) {
		case VALV_OPT_BAD: return dflt;
		case VALV_OPT_NULL: *idx += 1; // fall-thru
		case VALV_OPT_OOB: return dflt;
...
VALUE*
valv_optional_ref_num(int count, VALUE **vals, int* idx, const char* name, VALUE* dflt) {
...
VALUE*
valv_get_num(const char* custname, int count, VALUE **vals, int idx, const char* name) {
	valv_type_require(custname, count, vals, idx, name, V_NUM);
	return vals[idx];
}
char*
...
VALUE*
valv_optional_ref_list(int count, VALUE **vals, int* idx, const char* name, VALUE* dflt) {
...
VALUE*
valv_optional_get_assoc(int count, VALUE **vals, int* idx, const char* name, VALUE* dflt) {
...
VALUE*
valv_get_assoc(const char* custname, int count, VALUE **vals, int idx, const char* name) {
...
u_pfe_execvp(char *UNUSED(name), int count, VALUE **vals)
{
	...
	char**		args;
	if (!(args = malloc(sizeof(char*) * args_list->v_list->l_count))) math_error("%s: "__FILE__": %d: malloc: %s", custname, __LINE__, strerror(errno));
	...
	if (execvp(path, args)) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
u_pfe_dup(char *UNUSED(name), int count, VALUE **vals)
{
	...
	int fd = dup(
		valv_get_num_long(custname, count, vals, 0, "source"));
	
	if (fd < 0) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
u_pfe_close(char *UNUSED(name), int count, VALUE **vals)
{
	...
	if (e) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
u_pfe_write(char *UNUSED(name), int count, VALUE **vals)
{
	...
	ssize_t written = write(
		valv_get_num_long(custname, count, vals, 0, "fd"),
		strp->v_str->s_str,
		strp->v_str->s_len);
	
	if (written < 0) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
u_pfe_read(char *UNUSED(name), int count, VALUE **vals)
{
	...
	int strl = count < 2 ? u_pfe_read_STRL_DFLT : valv_get_num_long(custname, count, vals, 1, "count");
	...
	if (r < 0) { free(str); math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno)); }
...
optional_valv_ref_list2fd_set(const char* custname, int count, VALUE **vals, int* idx, const char* name, fd_set* fds, int* setsize) {
	valv_opt chk = valv_optional_ref_type_check(count, vals, *idx, name, V_LIST);
	switch (chk) {
		case VALV_OPT_BAD: return NULL;
		case VALV_OPT_NULL: *idx += 1; // fall-thru
		case VALV_OPT_OOB: return NULL;
		case VALV_OPT_REF_NULL: math_error("%s: argument %d (%s) address of type null is not allowed", custname, *idx, name);
		case VALV_OPT_GOOD: ; // fall-thru
	}
	VALUE* list = vals[*idx]->v_addr;
	*idx += 1;
...
u_pfe_select(char *UNUSED(name), int count, VALUE **vals)
{
	...
	int ssz = 0;
	fd_set rds; FD_ZERO(&rds); VALUE* rdl = optional_valv_ref_list2fd_set(custname, count, vals, &v, "reading"		,&rds,&ssz);
	fd_set wts; FD_ZERO(&wts); VALUE* wtl = optional_valv_ref_list2fd_set(custname, count, vals, &v, "writing"		,&wts,&ssz);
	fd_set ers; FD_ZERO(&ers); VALUE* erl = optional_valv_ref_list2fd_set(custname, count, vals, &v, "exceptional"	,&ers,&ssz);
	
	if (!pfe_select_TIMEOUT_DFLT) pfe_select_TIMEOUT_DFLT = alloc_num(itoq(-1));
	
	VALUE* tv = valv_optional_get_num(count, vals, &v, "timeout", pfe_select_TIMEOUT_DFLT);
	...
	if (r < 0) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
u_pfe_poll(char *UNUSED(name), int count, VALUE **vals)
{
	...
	int v = 2;
	VALUE* tv = valv_optional_get_num(count, vals, &v, "timeout", u_pfe_poll_TIMEOUT_DFLT);
	NUMBER* tn = tv->v_num;
	if (!qisint(tn)) math_error("%s: argument 2 (timeout) must be integer", custname);
	int t = qtoi(tn);
	
	LIST* il = iv->v_list;
	struct pollfd* pollfds = malloc(il->l_count*sizeof(struct pollfd)); //cspell:ignore pollfds
	...
	if (r < 0) { free(pollfds); math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno)); }
...
u_pfe_wait4(char *UNUSED(name), int count, VALUE **vals)
{
	...
	VALUE*	stt_assoc	= valv_ref_assoc(custname,		count, vals,  v++,	"status"			); // stat in
	...
	if (!pid_num && opt_list	) math_error("%s: argument 1 (pid) being null doesn't support options list",	custname);
	if (!pid_num && usg_assoc	) math_error("%s: argument 1 (pid) being null doesn't support usage assoc",		custname);
	...
	if (opt_list) {
		...
		for (;
			...
			else math_error("%s: options list element %d must be one of \"nohang\" or \"untraced\" (%s given)", custname, s, opt_str);
		}
	}
	...
	pid_t r = pid_num ? wait4(qtoi(pid_num->v_num), &stt, opts, usg_p) : wait(&stt);
	
	if (0 > r) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
u_pfe_pfe(char *UNUSED(name), int count, VALUE **vals)
{
	...
	// in, out, err arrays
	int ia[2];if (pipe(ia)) math_error("%s: "__FILE__": %d: (in) pipe: %s", custname, __LINE__, strerror(errno));
	int oa[2];if (pipe(oa)) math_error("%s: "__FILE__": %d: (out) pipe: %s", custname, __LINE__, strerror(errno));
	int ea[2];if (pipe(ea)) math_error("%s: "__FILE__": %d: (err) pipe: %s", custname, __LINE__, strerror(errno));
	// in, out, err values
	...
	if (child) {
		...
		if (cci) math_error("%s: "__FILE__": %d: child: (parent in) close: %s", custname, __LINE__, strerror(cci));
		if (cco) math_error("%s: "__FILE__": %d: child: (parent out) close: %s", custname, __LINE__, strerror(cco));
		if (cce) math_error("%s: "__FILE__": %d: child: (parent err) close: %s", custname, __LINE__, strerror(cce));
		...
		if (pci) math_error("%s: "__FILE__": %d: parent: (child in) close: %s", custname, __LINE__, strerror(pci));
		if (pco) math_error("%s: "__FILE__": %d: parent: (child out) close: %s", custname, __LINE__, strerror(pco));
		if (pce) math_error("%s: "__FILE__": %d: parent: (child err) close: %s", custname, __LINE__, strerror(pce));
		
		if (dup2(ia[0], 0)!=0) math_error("%s: "__FILE__": %d: mismatch: (in) dup2: %s", custname, __LINE__, strerror(errno));
		if (dup2(oa[1], 1)!=1) math_error("%s: "__FILE__": %d: mismatch: (out) dup2: %s", custname, __LINE__, strerror(errno));
		if (dup2(ea[1], 2)!=2) math_error("%s: "__FILE__": %d: mismatch: (err) dup2: %s", custname, __LINE__, strerror(errno));
		...
		if (execvp(args[0], args)) math_error("%s: "__FILE__": %d: execvp: %s", custname, __LINE__, strerror(errno));
...
u_pfe_pwrite(char *UNUSED(name), int count, VALUE **vals)
{
	...
	if (w < 0) math_error("%s: "__FILE__": %d: write: %s", custname, __LINE__, strerror(errno));

	int e = close(fd);
	
	if (e) math_error("%s: "__FILE__": %d: close: %s", custname, __LINE__, strerror(errno));
...
u_pfe_pread(char *UNUSED(name), int count, VALUE **vals)
{
	...
	fd_set nvm; FD_ZERO(&nvm); // never mind
	fd_set rbs; FD_ZERO(&rbs); // read base set
	int ssz = 0;
	FD_SET(out, &rbs); if (out > ssz) ssz = out;
	FD_SET(err, &rbs); if (err > ssz) ssz = err;
	
	// Keep track of rbs
	bool eoo = NULL; // end of out
	bool eoe = NULL; // end of err
	
	char* o = malloc(pfe_pfe_SIZE_INIT*sizeof(char)); o[0] = '\0';
	char* e = malloc(pfe_pfe_SIZE_INIT*sizeof(char)); e[0] = '\0';
	...
	while (!eoo || !eoe) {
		...
		if (r < 0) math_error("%s: "__FILE__": %d: select: %s", custname, __LINE__, strerror(errno));
	...
	if (pco) math_error("%s: "__FILE__": %d: (out) close: %s", custname, __LINE__, strerror(pco));
	if (pce) math_error("%s: "__FILE__": %d: (err) close: %s", custname, __LINE__, strerror(pce));
...
u_vadd_getcwd(char *UNUSED(name), int UNUSED(count), VALUE **UNUSED(vals))
{
	...
	char* buf = getcwd(NULL, -1); // doing equiv(malloc) COULD use MAXPATHLEN from <sys/param.h> and s/makestring/makenewstring/ below I guess
	if (!buf) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
/*ARGSUSED*/
VALUE
u_vadd_inputname(char *UNUSED(name), int UNUSED(count), VALUE **UNUSED(vals)) //cspell:ignore inputname
{
	const char* UNUSED(custname) = "inputname";
	
	char* buf = inputname();
	if (!buf) buf = inputisterminal() ? "<terminal>" : "<anonymous>"; //cspell:ignore inputisterminal
...
u_vadd_basename(char *UNUSED(name), int count, VALUE **vals)
{
	...
	char buf[MAXPATHLEN];
	if (!basename_r(strp->v_str->s_str, buf)) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
...
u_vadd_dirname(char *UNUSED(name), int count, VALUE **vals)
{
	...
	char buf[MAXPATHLEN];
	if (!dirname_r(strp->v_str->s_str, buf)) math_error("%s: "__FILE__": %d: %s", custname, __LINE__, strerror(errno));
