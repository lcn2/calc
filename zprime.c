    ...
    if (x <= MAX_PI10B) {
        ...
        if (x < MAX_MAP_PRIME) {
            ...
            if (x & 0x200) {
                ...
                count = pi10b[x >> 10];
                for (i = next_prime(x & (~0x3ff)); i <= x; i = next_prime(i)) {
                    ++count;
                }
            }
        }
        ...
        if (x & 0x20000) {
            ...
            if (top > MAX_SM_PRIME) {
                if (x < MAX_SM_PRIME) {
                    for (i = next_prime(x); i < MAX_SM_PRIME; i = next_prime(i)) {
                        --count;
                    }
                    --count;
                }
            ...
            for (i = next_prime(x & (~0x3ffff)); i <= x; i = next_prime(i)) {
                ++count;
            }
        }
    }
...
zredcprimetest(ZVALUE z, long count, ZVALUE skip)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        case 0:
            ...
            } while (!zcmp(zbase, rp->one) || !zcmp(zbase, zredcm1));
            break;
        case 1:
            if (i == 0) {
                break;
