...
static void o_getvalue(FUNC *fp);
...
o_argvalue(FUNC *fp, int argcount, VALUE *args)
{
    ...
    if ((vp->v_type != V_NUM) || qisneg(vp->v_num) || qisfrac(vp->v_num)) {
...
o_indexaddr(FUNC *UNUSED(fp), long dim, long writeflag)
{
    ...
    case V_BLOCK:
        ...
        if (zge31b(indices[0].v_num->num) || zisneg(indices[0].v_num->num)) {
...
o_swap(FUNC *UNUSED(fp))
{
    ...
    if (v1->v_type == V_OCTET && v2->v_type == V_OCTET) {
        if (v1->v_octet != v2->v_octet && ((v1->v_subtype | v2->v_subtype) & (V_NOCOPYTO | V_NOCOPYFROM))) {
        ...
        *v2->v_octet = usb;
        ...
        v1 = v1->v_addr;
        v2 = v2->v_addr;
        if (v1 != v2 && ((v1->v_subtype | v2->v_subtype) & (V_NOASSIGNTO | V_NOASSIGNFROM))) {
...
o_abs(FUNC *UNUSED(fp))
{
    ...
    if ((v1->v_type != V_NUM) || (v2->v_type != V_NUM) || !qispos(v2->v_num)) {
        absvalue(v1, v2, &tmp);
        freevalue(stack--);
        freevalue(stack);
        *stack = tmp;
...
o_links(FUNC *UNUSED(fp))
{
    ...
    switch (vp->v_type) {
    case V_NUM:
        links = vp->v_num->links;
        break;
    case V_COM:
        links = vp->v_com->links;
        break;
    case V_STR:
        links = vp->v_str->s_links;
        break;
    default:
        freevalue(stack);
        return;
...
o_le(FUNC *UNUSED(fp))
{
    ...
    stack->v_subtype = V_NOSUBTYPE;
    if (tmp.v_type == V_NUM) {
        stack->v_num = !qispos(tmp.v_num) ? qlink(&_qone_) : qlink(&_qzero_);
...
o_ge(FUNC *UNUSED(fp))
{
    ...
    stack->v_subtype = V_NOSUBTYPE;
    if (tmp.v_type == V_NUM) {
        stack->v_num = !qisneg(tmp.v_num) ? qlink(&_qone_) : qlink(&_qzero_);
    } else if (tmp.v_type == V_COM) {
        stack->v_num = qlink(&_qzero_);
    } else {
        stack->v_type = V_NULL;
...
o_quit(FUNC *fp, long index)
{
    ...
    if (inputisterminal() && !strcmp(fp->f_name, "*")) {
        ...
        if (calc_use_scanerr_jmpbuf != 0) {
            ...
            fprintf(stderr, "calc_scanerr_jmpbuf not setup, exiting code 50\n");
...
o_setconfig(FUNC *UNUSED(fp))
{
    ...
    if (type < 0) {
        math_error("Unknown config name \"%s\"", v1->v_str->s_str);
        not_reached();
    }
...
o_getconfig(FUNC *UNUSED(fp))
{
    ...
    if (type < 0) {
        math_error("Unknown config name \"%s\"", vp->v_str->s_str);
        not_reached();
    }
    freevalue(stack);
...
showsizes(void)
{
    ...
    printf("\tLISTELEM\t%4zu\n", sizeof(LISTELEM));
    printf("\tOBJECT\t\t%4zu\n", sizeof(OBJECT));
    printf("\tOBJECTACTIONS\t%4zu\n", sizeof(OBJECTACTIONS));
    printf("\tASSOC\t\t%4zu\n", sizeof(ASSOC));
    printf("\tASSOCELEM\t%4zu\n", sizeof(ASSOCELEM));
    printf("\tBLOCK\t\t%4zu\n", sizeof(BLOCK));
    printf("\tNBLOCK\t\t%4zu\n", sizeof(NBLOCK));
    printf("\tCONFIG\t\t%4zu\n", sizeof(CONFIG));
    printf("\tFILEIO\t\t%4zu\n", sizeof(FILEIO));
    ...
    printf("\tBASEB\t\t%4d\n", BASEB);
#endif
#if defined(HALF_BITS)
    printf("\tHALF_BITS\t%4d\n", HALF_BITS);
#endif
#if defined(HALF_LEN)
    printf("\tHALF_LEN\t%4zu\n", HALF_LEN);
#endif
#if defined(FULL_BITS)
    printf("\tFULL_BITS\t%4d\n", FULL_BITS);
#endif
#if defined(FULL_LEN)
    printf("\tFULL_LEN\t%4zu\n", FULL_LEN);
...
#endif
#if defined(ADDR_REDUCT)
    printf("\tADDR_REDUCT\t%4d\n", ADDR_REDUCT);
#endif
#if defined(MAXDATA_LOG2)
...
static struct opcode opcodes[MAX_OPCODE + 1] = {
    {{.func_nul = o_nop}, OPNUL, "NOP"}, /* no operation */

    {{.func_loc = o_localaddr}, OPLOC, "LOCALADDR"}, /* address of local variable */

    {{.func_glb = o_globaladdr}, OPGLB, "GLOBALADDR"}, /* address of global variable */

    {{.func_par = o_paramaddr}, OPPAR, "PARAMADDR"}, /* address of parameter variable */

    {{.func_loc = o_localvalue}, OPLOC, "LOCALVALUE"}, /* value of local variable */

    {{.func_glb = o_globalvalue}, OPGLB, "GLOBALVALUE"}, /* value of global variable */

    {{.func_par = o_paramvalue}, OPPAR, "PARAMVALUE"}, /* value of parameter variable */

    {{.func_one = o_number}, OPONE, "NUMBER"}, /* constant real numeric value */

    {{.func_two = o_indexaddr}, OPTWO, "INDEXADDR"}, /* array index address */

    {{.func_nul = o_printresult}, OPNUL, "PRINTRESULT"}, /* print result of top-level expression */

    {{.func_nul = o_assign}, OPNUL, "ASSIGN"}, /* assign value to variable */

    {{.func_nul = o_add}, OPNUL, "ADD"}, /* add top two values */

    {{.func_nul = o_sub}, OPNUL, "SUB"}, /* subtract top two values */

    {{.func_nul = o_mul}, OPNUL, "MUL"}, /* multiply top two values */

    {{.func_nul = o_div}, OPNUL, "DIV"}, /* divide top two values */

    {{.func_nul = o_mod}, OPNUL, "MOD"}, /* take mod of top two values */

    {{.func_nul = o_save}, OPNUL, "SAVE"}, /* save value for later use */

    {{.func_nul = o_negate}, OPNUL, "NEGATE"}, /* negate top value */

    {{.func_nul = o_invert}, OPNUL, "INVERT"}, /* invert top value */

    {{.func_nul = o_int}, OPNUL, "INT"}, /* take integer part */

    {{.func_nul = o_frac}, OPNUL, "FRAC"}, /* take fraction part */

    {{.func_nul = o_numerator}, OPNUL, "NUMERATOR"}, /* take numerator */

    {{.func_nul = o_denominator}, OPNUL, "DENOMINATOR"}, /* take denominator */

    {{.func_nul = o_duplicate}, OPNUL, "DUPLICATE"}, /* duplicate top value */

    {{.func_nul = o_pop}, OPNUL, "POP"}, /* pop top value */

    {{.func_ret = o_return}, OPRET, "RETURN"}, /* return value of function */

    {{.func_jmp = o_jumpz}, OPJMP, "JUMPZ"}, /* jump if value zero */

    {{.func_jmp = o_jumpnz}, OPJMP, "JUMPNZ"}, /* jump if value nonzero */

    {{.func_jmp = o_jump}, OPJMP, "JUMP"}, /* jump unconditionally */

    {{.func_two = o_usercall}, OPTWO, "USERCALL"}, /* call a user function */

    {{.func_nul = o_getvalue}, OPNUL, "GETVALUE"}, /* convert address to value */

    {{.func_nul = o_eq}, OPNUL, "EQ"}, /* test elements for equality */

    {{.func_nul = o_ne}, OPNUL, "NE"}, /* test elements for inequality */

    {{.func_nul = o_le}, OPNUL, "LE"}, /* test elements for < =  */

    {{.func_nul = o_ge}, OPNUL, "GE"}, /* test elements for > =  */

    {{.func_nul = o_lt}, OPNUL, "LT"}, /* test elements for < */

    {{.func_nul = o_gt}, OPNUL, "GT"}, /* test elements for > */

    {{.func_nul = o_preinc}, OPNUL, "PREINC"}, /* add one to variable (++x) */

    {{.func_nul = o_predec}, OPNUL, "PREDEC"}, /* subtract one from variable (--x) */

    {{.func_nul = o_postinc}, OPNUL, "POSTINC"}, /* add one to variable (x++) */

    {{.func_nul = o_postdec}, OPNUL, "POSTDEC"}, /* subtract one from variable (x--) */

    {{.func_one = o_debug}, OPONE, "DEBUG"}, /* debugging point */

    {{.func_one = o_print}, OPONE, "PRINT"}, /* print value */

    {{.func_nul = o_assignpop}, OPNUL, "ASSIGNPOP"}, /* assign to variable and pop it */

    {{.func_nul = o_zero}, OPNUL, "ZERO"}, /* put zero on the stack */

    {{.func_nul = o_one}, OPNUL, "ONE"}, /* put one on the stack */

    {{.func_nul = o_printeol}, OPNUL, "PRINTEOL"}, /* print end of line */

    {{.func_nul = o_printspace}, OPNUL, "PRINTSPACE"}, /* print a space */

    {{.func_one = o_printstring}, OPONE, "PRINTSTR"}, /* print constant string */

    {{.func_nul = o_dupvalue}, OPNUL, "DUPVALUE"}, /* duplicate value of top value */

    {{.func_nul = o_oldvalue}, OPNUL, "OLDVALUE"}, /* old value from previous calc */

    {{.func_nul = o_quo}, OPNUL, "QUO"}, /* integer quotient of top values */

    {{.func_nul = o_power}, OPNUL, "POWER"}, /* value raised to a power */

    {{.func_one = o_quit}, OPONE, "QUIT"}, /* quit program */

    {{.func_two = o_call}, OPTWO, "CALL"}, /* call built-in routine */

    {{.func_nul = o_getepsilon}, OPNUL, "GETEPSILON"}, /* get allowed error for calculations */

    {{.func_nul = o_and}, OPNUL, "AND"}, /* arithmetic and or top two values */

    {{.func_nul = o_or}, OPNUL, "OR"}, /* arithmetic or of top two values */

    {{.func_nul = o_not}, OPNUL, "NOT"}, /* logical not or top value */

    {{.func_nul = o_abs}, OPNUL, "ABS"}, /* absolute value of top value */

    {{.func_nul = o_sgn}, OPNUL, "SGN"}, /* sign of number */

    {{.func_nul = o_isint}, OPNUL, "ISINT"}, /* whether number is an integer */

    {{.func_jmp = o_condorjump}, OPJMP, "CONDORJUMP"}, /* conditional or jump */

    {{.func_jmp = o_condandjump}, OPJMP, "CONDANDJUMP"}, /* conditional and jump */

    {{.func_nul = o_square}, OPNUL, "SQUARE"}, /* square top value */

    {{.func_one = o_string}, OPONE, "STRING"}, /* string constant value */

    {{.func_nul = o_isnum}, OPNUL, "ISNUM"}, /* whether value is a number */

    {{.func_nul = o_undef}, OPNUL, "UNDEF"}, /* load undefined value on stack */

    {{.func_nul = o_isnull}, OPNUL, "ISNULL"}, /* whether value is the null value */

    {{.func_arg = o_argvalue}, OPARG, "ARGVALUE"}, /* load value of arg (parameter) n */

    {{.func_one = o_matcreate}, OPONE, "MATCREATE"}, /* create matrix */

    {{.func_nul = o_ismat}, OPNUL, "ISMAT"}, /* whether value is a matrix */

    {{.func_nul = o_isstr}, OPNUL, "ISSTR"}, /* whether value is a string */

    {{.func_nul = o_getconfig}, OPNUL, "GETCONFIG"}, /* get value of configuration parameter */

    {{.func_nul = o_leftshift}, OPNUL, "LEFTSHIFT"}, /* left shift of integer */

    {{.func_nul = o_rightshift}, OPNUL, "RIGHTSHIFT"}, /* right shift of integer */

    {{.func_jmp = o_casejump}, OPJMP, "CASEJUMP"}, /* test case and jump if not matched */

    {{.func_nul = o_isodd}, OPNUL, "ISODD"}, /* whether value is odd integer */

    {{.func_nul = o_iseven}, OPNUL, "ISEVEN"}, /* whether value is even integer */

    {{.func_nul = o_fiaddr}, OPNUL, "FIADDR"}, /* 'fast index' matrix address */

    {{.func_nul = o_fivalue}, OPNUL, "FIVALUE"}, /* 'fast index' matrix value */

    {{.func_nul = o_isreal}, OPNUL, "ISREAL"}, /* whether value is real number */

    {{.func_one = o_imaginary}, OPONE, "IMAGINARY"}, /* constant imaginary numeric value */

    {{.func_nul = o_re}, OPNUL, "RE"}, /* real part of complex number */

    {{.func_nul = o_im}, OPNUL, "IM"}, /* imaginary part of complex number */

    {{.func_nul = o_conjugate}, OPNUL, "CONJUGATE"}, /* complex conjugate */

    {{.func_one = o_objcreate}, OPONE, "OBJCREATE"}, /* create object */

    {{.func_nul = o_isobj}, OPNUL, "ISOBJ"}, /* whether value is an object */

    {{.func_nul = o_norm}, OPNUL, "NORM"}, /* norm of value (square of abs) */

    {{.func_one = o_elemaddr}, OPONE, "ELEMADDR"}, /* address of element of object */

    {{.func_one = o_elemvalue}, OPONE, "ELEMVALUE"}, /* value of element of object */

    {{.func_nul = o_istype}, OPNUL, "ISTYPE"}, /* whether types are the same */

    {{.func_nul = o_scale}, OPNUL, "SCALE"}, /* scale value by a power of two */

    {{.func_nul = o_islist}, OPNUL, "ISLIST"}, /* whether value is a list */

    {{.func_nul = o_swap}, OPNUL, "SWAP"}, /* swap values of two variables */

    {{.func_nul = o_issimple}, OPNUL, "ISSIMPLE"}, /* whether value is simple type */

    {{.func_nul = o_cmp}, OPNUL, "CMP"}, /* compare values returning -1, 0, 1 */

    {{.func_nul = o_setconfig}, OPNUL, "SETCONFIG"}, /* set configuration parameter */

    {{.func_nul = o_setepsilon}, OPNUL, "SETEPSILON"}, /* set allowed error for calculations */

    {{.func_nul = o_isfile}, OPNUL, "ISFILE"}, /* whether value is a file */

    {{.func_nul = o_isassoc}, OPNUL, "ISASSOC"}, /* whether value is an association */

    {{.func_sti = o_nop}, OPSTI, "INITSTATIC"}, /* once only code for static init */

    {{.func_one = o_eleminit}, OPONE, "ELEMINIT"}, /* assign element of matrix or object */

    {{.func_nul = o_isconfig}, OPNUL, "ISCONFIG"}, /* whether value is a configuration state */

    {{.func_nul = o_ishash}, OPNUL, "ISHASH"}, /* whether value is a hash state */

    {{.func_nul = o_isrand}, OPNUL, "ISRAND"}, /* whether value is a rand element */

    {{.func_nul = o_israndom}, OPNUL, "ISRANDOM"}, /* whether value is a random element */

    {{.func_one = o_show}, OPONE, "SHOW"}, /* show current state data */

    {{.func_nul = o_initfill}, OPNUL, "INITFILL"}, /* initially fill matrix */

    {{.func_nul = o_assignback}, OPNUL, "ASSIGNBACK"}, /* assign in reverse order */

    {{.func_nul = o_test}, OPNUL, "TEST"}, /* test that value is "nonzero" */

    {{.func_nul = o_isdefined}, OPNUL, "ISDEFINED"}, /* whether a string names a function */

    {{.func_nul = o_isobjtype}, OPNUL, "ISOBJTYPE"}, /* whether a string names an object type */

    {{.func_nul = o_isblock}, OPNUL, "ISBLK"}, /* whether value is a block */

    {{.func_nul = o_ptr}, OPNUL, "PTR"}, /* octet pointer */

    {{.func_nul = o_deref}, OPNUL, "DEREF"}, /* dereference an octet pointer */

    {{.func_nul = o_isoctet}, OPNUL, "ISOCTET"}, /* whether a value is an octet */

    {{.func_nul = o_isptr}, OPNUL, "ISPTR"}, /* whether a value is a pointer */

    {{.func_nul = o_setsaveval}, OPNUL, "SAVEVAL"}, /* enable or disable saving */

    {{.func_nul = o_links}, OPNUL, "LINKS"}, /* links to number or string */

    {{.func_nul = o_bit}, OPNUL, "BIT"}, /* whether bit is set */

    {{.func_nul = o_comp}, OPNUL, "COMP"}, /* complement value */

    {{.func_nul = o_xor}, OPNUL, "XOR"}, /* xor (~) of values */

    {{.func_nul = o_highbit}, OPNUL, "HIGHBIT"}, /* highbit of value */

    {{.func_nul = o_lowbit}, OPNUL, "LOWBIT"}, /* lowbit of value */

    {{.func_nul = o_content}, OPNUL, "CONTENT"}, /* unary hash op */

    {{.func_nul = o_hashop}, OPNUL, "HASHOP"}, /* binary hash op */

    {{.func_nul = o_backslash}, OPNUL, "BACKSLASH"}, /* unary backslash op */

    {{.func_nul = o_setminus}, OPNUL, "SETMINUS"}, /* binary backslash op */

    {{.func_nul = o_plus}, OPNUL, "PLUS"}, /* unary + op */

    {{.func_jmp = o_jumpnn}, OPJMP, "JUMPNN"}, /* jump if non-null */

    {{.func_one = o_abort}, OPONE, "ABORT"} /* abort operation */
};
...
calculate(FUNC *fp, int argcount)
{
    ...
    while (go) {
        ...
        case OPTWO: /* two extra integer arguments */
            (*op->o_func.func_two)(fp, fp->f_opcodes[pc], fp->f_opcodes[pc + 1]);
            pc += 2;
            break;
...
freenumbers(FUNC *fp)
{
    ...
    for (pc = 0; pc < fp->f_opcodecount;) {
        ...
        case OPONE:
            ...
            case OP_STRING:
            case OP_QUIT:
                freestringconstant((long)fp->f_opcodes[pc]);
                break;
            }
            /*FALLTHRU*/
        case OPLOC:
