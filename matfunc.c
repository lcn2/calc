...
matpowi(MATRIX *m, NUMBER *q)
{
    ...
    if (m->m_dim == 2 && (m->m_max[0] - m->m_min[0] != m->m_max[1] - m->m_min[1])) {
...
matinit(MATRIX *m, VALUE *v1, VALUE *v2)
{
    ...
    *res = *m;
     ...
     * firewall
     */
    if (v2 && ((res->m_dim != 2) || ((res->m_max[0] - res->m_min[0]) != (res->m_max[1] - res->m_min[1])))) {
...
matprint(MATRIX *m, long max_print)
{
    ...
    math_fmt("] (%ld element%s, %ld nonzero)", fullsize, (fullsize == 1) ? "" : "s", count);
    if (max_print <= 0) {
        return;
    }
